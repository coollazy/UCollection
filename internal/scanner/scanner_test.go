...
func resetDB(t *testing.T, pool *store.Pool) {
	t.Helper()
	ctx := context.Background()
	_, err := pool.Exec(ctx, `TRUNCATE master_wallets, orders, order_state_transitions, incoming_transactions, webhook_deliveries, scan_checkpoint RESTART IDENTITY CASCADE`)
	if err != nil {
