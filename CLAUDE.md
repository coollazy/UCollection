...
兩套本機 docker-compose 堆疊各自獨立、互不影響：

- **port 8080**（`.env`）：使用者自己驗收測試用。
- **port 8081**（`.env.stage08test2`，compose project `ucollection-test2`）：Claude 開發中驗證用。

細節、當前狀態、啟動指令見 `docs/進度.md`（隨開發進度更新，實際情況以該處最新記錄為準）。
...
8. **API Key 的 `secret` 與 Webhook 的 `webhook_secret` 明碼存資料庫是刻意決策**（HMAC 驗簽需要伺服器重算，雜湊值無法逆推），不要把它「修正」成雜湊存——那會讓 HMAC 機制整個失效。反之，session token 一律只存 hash，不存明碼。API Key 本身的 `key` 只存 hash（`key_hash`，供查找比對用），額外存前4碼+後4碼明碼片段（`key_hint`）供後台列表識別用途——僅供識別、8碼片段不足以重建43碼的完整金鑰，也不能拿來通過驗證；其餘部分僅在產生當下的 HTTP 回應中明碼回傳一次（見 [ADR-0019](docs/adr/0019-API-Key片段供後台識別.md)，延伸 [ADR-0010](docs/adr/0010-API-Key與Webhook-secret明碼存.md)，非推翻）。
9. **`/tron-proxy/...`、手動改判、代收主錢包切換、API Key 重新產生、Webhook URL/secret 變更、金額容許誤差百分比設定，一律套用 `RequireTOTPCode`（每次送出都要求當下輸入TOTP驗證碼，不設新鮮度窗口，見 [ADR-0016](docs/adr/0016-高風險操作每次要求TOTP不設新鮮度窗口.md)）**，不能只掛一般登入 session。判準見技術架構設計第11節「權限分級理由」——凡是攻擊者僅取得 session cookie（未取得2FA裝置）就能造成資金劫持、身分冒用、或誤導商戶財務判斷的操作，都屬此類（金額容許誤差百分比屬於「誤導商戶財務判斷」：調高此值會讓之後新訂單的短付更容易被系統自動判定為COMPLETED，階段07全系統審查發現此欄位曾跟其他低風險參數共用同一個RequireSession路由，已拆分修正）。新增後台寫入端點時，先用這個判準檢查是否也該疊加。**例外（批次寬限）**：`/tron-proxy/...` 這 4 條組交易/廣播端點因單次操作含批次多筆，改套用 `RequireTOTPCodeOrRecentStepUp`——批次內第一次仍要求當下有效驗證碼，成功後於批次寬限窗內同 session 的後續呼叫免重輸（一般 15 分鐘；手動歸集編排流程 30 分鐘，見 [ADR-0016](docs/adr/0016-高風險操作每次要求TOTP不設新鮮度窗口.md)/[0017](docs/adr/0017-手動歸集流程重構自動編排與精省手續費.md)）；其餘高風險操作維持每次送出都要求當下輸入驗證碼。
...
- 任何會讓「安全鐵律」清單變動的改動。
- 需要新增/移除某個 `RequireTOTPCode` 保護時——判斷邏輯要能重新推導出「攻擊者僅取得 session cookie 能造成什麼後果」，想不清楚就先問。
